...
def update_comment(connection, comment_id: int, user_email: str, new_text: str) -> bool:
    """
    ...
    try:
        ...
        # 2. ADIM: Yorumun sahibi kontrolü ve güncelleme
        check_query = """
        ...
        if comment['user_id'] != user_id:
            print(f"HATA: Bu yorumu sadece sahibi güncelleyebilir.")
            cursor.close()
            return False

        # 3. ADIM: Yorumu güncelle
        update_query = """
            UPDATE activity_comments 
            SET text = %s 
            WHERE comment_id = %s AND user_id = %s
        """
        cursor.execute(update_query, (new_text, comment_id, user_id))
        ...
        print(f"HATA: Yorum güncellenirken SQL hatası: {e}")
        if connection:
            connection.rollback()
        return False
